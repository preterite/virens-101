---
title: "VIRENS 101: Consolidated Semester Schedule"
aliases: ["VIRENS 101: Consolidated Semester Schedule"]
linter-yaml-title-alias: "VIRENS 101: Consolidated Semester Schedule"
created: 2026-01-31 14:12
modified: 2026-02-12 16:33
---

# VIRENS 101: Consolidated Semester Schedule

**Version**: 1.0
**Date**: 2026-01-23
**Status**: Locked for weekly index generation

---

## Overview

- **Semester**: 15 weeks (abstract, adaptable to any semester)
- **Time structure**: 6 blocks/week × 25 minutes = 150 minutes/week
- **Total blocks**: 90 (62 available for instruction after subtractions)
- **Fragment target**: 40-50 by Week 12

---

## Movement Structure

| Movement | Weeks | Focus | W/P/R Emphasis |
|----------|-------|-------|----------------|
| M1: Surfacing | 1-3 | Expansion, capture, freewriting | W-heavy |
| M2: Naming | 4-7 | Sources, structured feedback, categorization | Balanced W-P-R |
| M3: Operating at Scale | 8-11 | Corpus management, consolidation | W+R heavy |
| M4: Synthesis | 12-15 | Portfolio assembly, polish, selection | R-heavy |

---

## Difficulty Reading Pairs

Students choose **one pair minimum** and complete difficulty entries on both readings:

| Pair | Readings | Weeks | Theme |
|------|----------|-------|-------|
| **A** | Salvatori + Alexander | 3 and 7 | Failure as Diagnostic |
| **B** | Meadows + Star | 5 and 10 | Seeing Invisible Systems |

---

## Peer Review Cycles

| Cycle | Week | Assignment | Protocol | Points |
|-------|------|------------|----------|--------|
| 1 | 5 | Literacy Narrative | Generous (SPQ) | 40 |
| 2 | 6 | Spec Document | Generous (SPQ) | 40 |
| 3 | 7 | Constraint Audit | Evaluative (DES) | 40 |
| 4 | 8 | BEAM Source Entries | Evaluative (DES) | 40 |
| 5 | 10 | BEAM Portfolio Framing | Generous (SPQ) | 40 |
| 6 | 12 | Documented Inquiry | Evaluative (DES) | 40 |

**Total**: 240 points (lowest 1 dropped)

**Reflective One-Pagers** (4 total): Due after Cycles 1, 4, 5, 6 (Weeks 5, 8, 10, 12)

---

## Assignment Due Weeks

| Assignment | Due Week | Peer Review | Instructor Feedback |
|------------|----------|-------------|---------------------|
| Literacy Narrative draft | End Wk 4 | Cycle 1 (Wk 5) | Wk 4 Block 6 |
| Spec Document draft | Mid Wk 6 | Cycle 2 (Wk 6) | Wk 6 Block 6 |
| Constraint Audit draft | Mid Wk 7 | Cycle 3 (Wk 7) | — |
| BEAM Source Entries (2+) | Mid Wk 8 | Cycle 4 (Wk 8) | — |
| Conference fragment (9.4) | Due TO Wk 9 | — | Conference meeting |
| BEAM Portfolio Framing | Mid Wk 10 | Cycle 5 (Wk 10) | — |
| BEAM Portfolio complete | End Wk 11 | — | Wk 11 Block 4 |
| Documented Inquiry draft | Mid Wk 12 | Cycle 6 (Wk 12) | — |
| Process Archaeology | Wk 13 | — | Wk 13 Block 5 |
| Final Portfolio | Wk 15 | — | — |

---

## Tech Instruction Sequence

| Block | Week | Activity | Duration |
|-------|------|----------|----------|
| T1 | 2 | Index cards + OneDrive folder setup | 25 min |
| T2 | 2 | Eli Review quick intro (ungraded) | 25 min |
| T3 | 5 | Zotero basics (library, part 1) | 25 min |
| T4 | 5 | Zotero practice (library, part 2) | 25 min |
| T5 | 8 | "What You're Automating" | 25 min |
| T6 | 13 | Optional: Advanced tools demo | 25 min |

**Touch Reminders** (5-10 min, embedded):
- Week 6: Zotero check-in
- Week 7: Eli helpfulness discussion
- Week 10: Folder organization check
- Week 11: Process documentation review
- Week 12: Portfolio assembly logistics

---

## Instructor Prompts

| Prompt | Week.Block | Title | Movement |
|--------|------------|-------|----------|
| 1.2 | 1.3 | Technological Literacy | M1 |
| 1.4 | 1.4 | Campus Observation | M1 |
| 3.5 | 3.5 | Ong Revision | M1 |
| 4.3 | 4.3 | Materiality Circulation | M2 |
| 6.2 | 6.2 | Pop Culture Constraints | M2 |
| 7.5 | 7.5 | Experimental Form | M2 |
| 8.3 | 8.3 | Concept Tracing | M3 |
| 9.4 | 9.4 | Conference Questions | M3 (homework) |
| 12.3 | 12.3 | Portfolio Selection | M4 |
| 13.3 | 13.3 | Infrastructure Analysis | M4 |
| 15.2 | 15.2 | Transfer Celebration | M4 |

**Distribution**: M1 (3), M2 (3), M3 (2), M4 (3)
**Requirement**: Students complete 5-6 minimum (at least 1 in M1, 2 cumulative by end of M2)

---

## Reading Schedule

| Week | Reading(s) | Est. Pages | Notes |
|------|-----------|------------|-------|
| 1 | Kirschenbaum, "Textpocalypse" | ~5 | Contemporary hook |
| 2 | Ong + README examples | ~30-40 | Theory-practice: consciousness + systems setup |
| 3 | Salvatori, "Conversations with Texts" | ~15-20 | **Difficulty Pair A begins** |
| 4 | Downs, Rhetoric excerpt | ~15-20 | Rhetorical situation |
| 5 | Meadows, "Leverage Points" | ~15-20 | **Difficulty Pair B begins** |
| 6 | Library/Babel composite + Jackson | ~25-35 | Classification + experimental form |
| 7 | Alexander, "Goodness of Fit" | ~20-25 | **Difficulty Pair A completes**; Constraint Audit |
| 8 | Brooks, "No Silver Bullet" | ~15-20 | Complexity/scale; M3 begins |
| 9 | _None_ | — | Conference week |
| 10 | Star + Bush, "Memex" | ~30-40 | **Difficulty Pair B completes**; infrastructure |
| 11 | Ribes & Jackson, "Data Bite Man" | ~15-20 | Maintenance/labor |
| 12 | Norman, "Design of Everyday Things" | ~20-25 | Affordances/constraints |
| 13 | Bernstein, "Blood on the Cutting Room Floor" | ~10-15 | Revision as meaning-making |
| 14 | _None_ | — | Presentations week |
| 15 | _None_ | — | Final week |

---

## Week-by-Week Block Matrix

### Legend
- **W** = Writing block
- **P** = Peer review block
- **R** = Revision block
- **T** = Tech instruction block
- **I** = Instructor feedback moment
- **—** = Non-instructional
- **(touch)** = Brief tech reminder embedded

---

### MOVEMENT 1: SURFACING (Weeks 1-3)

#### Week 1
| Block | Type | Activity |
|-------|------|----------|
| 1 | W | Envelope exercise (hard stop at 25 min) |
| 2 | W | Envelope freewriting continues |
| 3 | W | First fragment capture _(Prompt 1.2 available)_ |
| 4 | W | Fragment development _(Prompt 1.4 available)_ |
| 5 | W | Fragment development |
| 6 | — | Syllabus review, course introduction |

**Reading**: Kirschenbaum (~5 pages)
**Tech**: None
**Fragment count**: Begin accumulation
**Note**: Syllabus review deliberately placed at end of week—students write first, constrain later (YOLO up, constrain down)

---

#### Week 2
| Block | Type | Activity |
|-------|------|----------|
| 1 | W | Freewriting / fragment work |
| 2 | T1 | Index cards + OneDrive folder setup |
| 3 | T2 | Eli Review quick intro (ungraded familiarization) |
| 4 | W | Writing with new systems (practice saving/naming) |
| 5 | W | Fragment development |
| 6 | W | Fragment development |

**Reading**: Ong [[@ong2012writi]] + README examples (~30-40 pages)
**Tech**: T1-T2 (folders, cards, Eli touch)
**Fragment count**: Continue accumulation

---

#### Week 3
| Block | Type | Activity |
|-------|------|----------|
| 1 | W | Fragment development toward Literacy Narrative |
| 2 | R | Self-revision of envelope material |
| 3 | W | Compression exercise (300→200→100) |
| 4 | P | Peer feedback on compression (in-class, informal) |
| 5 | R | Revision based on peer feedback _(Prompt 3.5 available)_ |
| 6 | W | Literacy Narrative draft completion |

**Reading**: Salvatori [[@salvatori1996conve]] (~15-20 pages) — **Difficulty Pair A begins** (completes Week 7 with Alexander)
**Tech**: Students practice folder system independently; dev log started
**Fragment count**: M1 target 8-12

---

### MOVEMENT 2: NAMING (Weeks 4-7)

#### Week 4
| Block | Type | Activity |
|-------|------|----------|
| 1 | W | Begin source-based writing (responding to course reading) |
| 2 | W | Continue source-based fragment development |
| 3 | P | In-class peer feedback on source-based fragments (informal) _(Prompt 4.3 available)_ |
| 4 | R | Revise Literacy Narrative for submission |
| 5 | W | Continue source-based writing |
| 6 | I | Instructor feedback on Literacy Narratives |

**Reading**: Downs [[@downs2007teaci]] (~15-20 pages)
**Tech**: No new tech; source-based writing with informal citation (pre-Zotero baseline)
**Due**: Literacy Narrative draft (end of week)

---

#### Week 5
| Block | Type | Activity |
|-------|------|----------|
| 1 | T3 | Zotero basics (library session, part 1) |
| 2 | T4 | Zotero practice (library session, part 2) |
| 3 | W | Source-based fragment with Zotero citation (immediate practice) |
| 4 | W | Spec Document brainstorming |
| 5 | P | **Eli Review Cycle 1: Literacy Narrative** |
| 6 | R | Process Eli feedback, begin revision planning |

**Reading**: Meadows [[@meadows2008lever]] (~15-20 pages) — **Difficulty Pair B begins** (completes Week 10 with Star)
**Tech**: T3-T4 (Zotero/library)
**Peer Review**: Cycle 1
**Reflective One-Pager**: Due after Cycle 1

---

#### Week 6
| Block | Type | Activity |
|-------|------|----------|
| 1 | W (touch) | Fragment work + (Zotero check-in: 5-10 min) |
| 2 | W | Spec Document development _(Prompt 6.2 available)_ |
| 3 | P | **Eli Review Cycle 2: Spec Document** |
| 4 | R | Revision based on Eli feedback |
| 5 | W | Constraint Audit brainstorming |
| 6 | I | Instructor feedback on Spec Documents |

**Reading**: Library/Babel composite + Jackson (~25-35 pages)
**Tech**: Zotero touch
**Peer Review**: Cycle 2
**Due**: Spec Document draft (mid-week)

---

#### Week 7
| Block | Type | Activity |
|-------|------|----------|
| 1 | W | Constraint Audit drafting |
| 2 | R (touch) | Revision work + (Eli helpfulness discussion: 10 min) |
| 3 | P | **Eli Review Cycle 3: Constraint Audit** |
| 4 | R | Revision based on feedback |
| 5 | W | Source genealogy: mapping how sources relate _(Prompt 7.5 available)_ |
| 6 | W | BEAM categorization: assigning source functions |

**Reading**: Alexander [[@alexander1964notet]] (~20-25 pages) — **Difficulty Pair A completes** (began Week 3 with Salvatori)
**Tech**: Eli helpfulness touch
**Peer Review**: Cycle 3
**Due**: Constraint Audit draft (mid-week)
**Fragment count**: M2 cumulative 20-28

---

### MOVEMENT 3: OPERATING AT SCALE (Weeks 8-11)

#### Week 8
| Block | Type | Activity |
|-------|------|----------|
| 1 | T5 | "What You're Automating" — digital transfer session |
| 2 | W | Corpus inventory: what do you have? |
| 3 | W | BEAM Portfolio entry drafting _(Prompt 8.3 available)_ |
| 4 | P | **Eli Review Cycle 4: BEAM Source Entries** |
| 5 | R | Process feedback, revision planning |
| 6 | R | Dev log update + conference prep (Prompt 9.4 assigned as homework) |

**Reading**: Brooks [[@brooks1986silve]] (~15-20 pages)
**Tech**: T5 (digital transfer)
**Peer Review**: Cycle 4
**Reflective One-Pager**: Due after Cycle 4
**Due**: BEAM Source Entries (2+ entries, mid-week)

---

#### Week 9 — CONFERENCE WEEK
| Block | Type | Activity |
|-------|------|----------|
| 1 | — | Individual conferences |
| 2 | — | Individual conferences |
| 3 | — | Individual conferences |
| 4 | — | Individual conferences |
| 5 | — | Individual conferences |
| 6 | — | Individual conferences |

**Reading**: None (catch-up/review)
**Tech**: None
**Homework TO this week**: Prompt 9.4 (Conference Questions & Format Choices)
**Conference structure**: 30 min each; student brings revisable fragment + questions; leaves with instructor email notes + direction toward next work
**Homework FROM this week**: Reading to support next draft/component

---

#### Week 10
| Block | Type | Activity |
|-------|------|----------|
| 1 | W | BEAM Portfolio entry drafting |
| 2 | R (touch) | Revision + (folder organization check: 5 min) |
| 3 | P | **Eli Review Cycle 5: BEAM Portfolio Framing** |
| 4 | R | Revision based on feedback |
| 5 | W | Continue BEAM Portfolio |
| 6 | W | Content mapping exercise |

**Reading**: Star [[@star1999ethnp]] + Bush [[@bush1945thinl]] (~30-40 pages) — **Difficulty Pair B completes** (began Week 5 with Meadows); Bush is regular reading, not difficulty pair
**Tech**: Folder check touch
**Peer Review**: Cycle 5
**Reflective One-Pager**: Due after Cycle 5
**Due**: BEAM Portfolio Framing (mid-week)

---

#### Week 11
| Block | Type | Activity |
|-------|------|----------|
| 1 | W | BEAM Portfolio completion |
| 2 | R (touch) | Revision + (process documentation review: 10 min) |
| 3 | R | Final BEAM Portfolio revision |
| 4 | I | Instructor feedback on BEAM Portfolios |
| 5 | W | Begin Documented Inquiry planning |
| 6 | R | Dev log update, system map draft |

**Reading**: Ribes [[@ribes2013datb]] & Jackson (~15-20 pages)
**Tech**: Process documentation touch
**Due**: BEAM Portfolio complete (end of week)
**Fragment count**: M3 cumulative 30-42

---

### MOVEMENT 4: SYNTHESIS (Weeks 12-15)

#### Week 12
| Block | Type | Activity |
|-------|------|----------|
| 1 | W | Documented Inquiry drafting |
| 2 | W | Continue Documented Inquiry |
| 3 | P | **Eli Review Cycle 6: Documented Inquiry** _(Prompt 12.3 available)_ |
| 4 | R (touch) | Revision based on feedback + (portfolio assembly logistics: 10 min) |
| 5 | R | Portfolio selection begins |
| 6 | R | Process Archaeology drafting |

**Reading**: Norman [[@norman2013desig]] (~20-25 pages)
**Tech**: Portfolio assembly touch
**Peer Review**: Cycle 6
**Reflective One-Pager**: Due after Cycle 6
**Due**: Documented Inquiry draft (mid-week)

---

#### Week 13
| Block | Type | Activity |
|-------|------|----------|
| 1 | R | Documented Inquiry revision |
| 2 | R | Process Archaeology development |
| 3 | T6 | Optional: Advanced tools demo (Voyant, OneNote, VIRENS) _(Prompt 13.3 available)_ |
| 4 | R | Portfolio assembly |
| 5 | I | Instructor feedback on portfolio drafts |
| 6 | R | Continue portfolio assembly |

**Reading**: Bernstein (~10-15 pages)
**Tech**: T6 (optional advanced demo)
**Due**: Process Archaeology

---

#### Week 14 — PRESENTATIONS
| Block | Type | Activity |
|-------|------|----------|
| 1 | — | Student presentations |
| 2 | — | Student presentations |
| 3 | — | Student presentations |
| 4 | — | Student presentations |
| 5 | — | Student presentations |
| 6 | — | Student presentations |

**Reading**: None
**Tech**: None

---

#### Week 15 — FINAL WEEK
| Block | Type | Activity |
|-------|------|----------|
| 1 | W | Envelope exercise: Week 15 writing (four parallel questions, 20 min timed) |
| 2 | W | Envelope return and comparison (open Week 1 envelopes, pair/class discussion) |
| 3 | W | Reflective cover letter drafting and portfolio assembly |
| 4 | — | Course evaluations (instructor exits room) |
| 5 | W | Transfer Proposal drafting |
| 6 | — | Final assembly and portfolio submission |

**Reading**: None
**Due**: Final Portfolio (includes Documented Inquiry, Process Archaeology, selected polished fragments, Transfer Proposal, reflective cover letter)
**Fragment count**: Final 40-50

---

## Summary Tables

### W/P/R Totals by Movement

| Type | M1 | M2 | M3 | M4 | Total |
|------|----|----|----|----|-------|
| W | 14 | 11 | 7 | 6 | 38 |
| P | 1 | 3 | 2 | 1 | 7 |
| R | 2 | 5 | 9 | 7 | 23 |
| T | 2 | 2 | 1 | 1 | 6 |
| I | 0 | 2 | 1 | 1 | 4 |
| — | 1 | 1 | 6 | 8 | 16 |

### Reading Page Load by Week

| Week | Pages | Status |
|------|-------|--------|
| 1 | ~5 | ✓ Light |
| 2 | ~30-40 | ✓ Moderate (tech week balances) |
| 3 | ~15-20 | ✓ Moderate |
| 4 | ~15-20 | ✓ Moderate |
| 5 | ~15-20 | ✓ Moderate |
| 6 | ~25-35 | ✓ Moderate |
| 7 | ~20-25 | ✓ Moderate |
| 8 | ~15-20 | ✓ Light |
| 9 | 0 | Conference |
| 10 | ~30-40 | ✓ Moderate (post-conference) |
| 11 | ~15-20 | ✓ Moderate |
| 12 | ~20-25 | ✓ Moderate |
| 13 | ~10-15 | ✓ Light |
| 14 | 0 | Presentations |
| 15 | 0 | Final |

---

## Abstract Flexibility Notes

**Holiday/flex blocks**: 2 blocks reserved for Monday/Tuesday holidays (every semester has ~2). Absorb into adjacent weeks or use as catch-up.

**Spring/Fall variation**: Conference week (9) and Presentations week (14) remain stable. Spring Break or Thanksgiving falls mid-semester; schedule accommodates either.

**Meeting pattern agnostic**: Designed at 25-min block level. Works as 1×150, 2×75, or 3×50.

---

## Files Needing Updates

| File | Change Needed |
|------|---------------|
| `peer-review-cycle4-beam-source-entries-criteria-course-v101.md` | Change "Week: 9" to "Week: 8" |
| `instructor-prompt-block9-4-conference-questions-course-v101.md` | Add note: homework TO conference week |
| Student reading -reasons- files | ✅ Difficulty pairs updated: Pair A = Salvatori + Alexander; Pair B = Meadows + Star |

---

## Next Steps

1. Generate weekly index notes (`i_week01/` through `i_week15/`)
2. Create semester schedule overview for `i_schedule/`
3. Update peer review and prompt files as noted above
4. Complete reading schedule documentation in `g_studentreadings/`

---

_Document consolidates decisions from schedule planning session 2026-01-23_
