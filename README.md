...
The course is built on 25-minute instructional blocks, six per week (150 minutes total), configurable as 1 x 150, 2 x 75, or 3 x 50 meeting patterns. Of 90 possible blocks, approximately 62 are available for instruction after accounting for holidays, conferences, and administrative time. Most writing happens in class (flipped classroom model); readings are homework.
...
VIRENS 101 is licensed under the [Creative Commons Attribution-ShareAlike 4.0 International License](LICENSE.txt).
...
Start with `a_description/course-overview-course-v101.md` for the student-facing overview, then explore `b_overview/` for the structural architecture. Assignment prompts in `f_assignments/` are designed to be copy-pasteable into an LMS with minimal adaptation. Consult `b_overview/folder-structure-guide-course-v101.md` for the full file naming and metadata schema.
